module "artifact_registry" {
  ...
  kms_key_id  = "projects/${[for entry in local.project_id : regex(local.regex_name, entry.name).name if entry.display_name == each.value.kms.kmsProject][0]}/locations/${each.value.kms.kmsLocation}/keyRings/${[for entry in local.project_id : regex(local.regex_name, entry.name).name if entry.display_name == each.value.project][0]}/cryptoKeys/${[for entry in local.project_id : regex(local.regex_name, entry.name).name if entry.display_name == each.value.project][0]}"
